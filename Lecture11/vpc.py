...
def get_or_create_vpc(ec2_client, vpc_name_tag):
  vpc = ec2_client.create_vpc(CidrBlock="10.0.0.0/16")  # You can adjust CIDR block if needed
  vpc_id = vpc["Vpc"]["VpcId"]
  waiter = ec2_client.get_waiter('vpc_available')
  waiter.wait(VpcIds=[vpc_id])
  print(f'vpc_id: {vpc_id}')
...
def create_subnet(ec2_client, vpc_id, cidr_block, subnet_name, availability_zone):
